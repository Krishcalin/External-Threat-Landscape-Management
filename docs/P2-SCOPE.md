...
### Two-thirds of KEV carries version data we can act on

40 KEV CVEs sampled at random, fetched live from the CVE Program API:
...
| placeholder or prose (`"n/a"`, `"Access 21.08.0.1, 21.08.0.0. Identity Manager 3.3.6…"`) | **32.5%** | no |

**≈67.5% reachable, ≈32.5% permanently not.** That third will stay
`PRODUCT_MATCH` however much data we ingest, and the product must say so rather
than implying the worklist eventually empties.

> The first attempt at this measurement returned 0% and the fault was mine —
> `affected_products()` takes the *whole* record and unwraps `containers.cna`
> itself, and I passed it the already-unwrapped container. Recorded because a
> 0% reading would have killed the workstream on a parsing error.
...
**Turns `PRODUCT_MATCH` into `VERSION_RANGE` for ~67.5% of KEV.**
...
1,674 records fetched one apiece from `cveawg.mitre.org`, measured working at
~0.12 s intervals. Written to `data/affected.json`, regenerated by
`tools/refresh_intel.py`, refusing to write a partial file.
...
not affected, could not determine — and say plainly that ~32.5% of the catalogue
carries no comparable version data at all, so that portion stays a worklist
permanently.
