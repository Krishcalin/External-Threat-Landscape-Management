...
Ported unchanged from OverWatch's `aws_qr.py`, same author, same job: showing an
`otpauth://` URI that Microsoft Authenticator, Google Authenticator, Authy and
1Password can scan. Written rather than depended on because this product's
collectors are deliberately dependency-light and a QR library is not worth the
budget — it does one thing, and the verification below is what makes hand-rolling
it defensible.
